...
#
#REFERENCES: W3Schools: "https://www.w3schools.com/python/ref_func_round.asp" 5%
...
protMass = 1.6726219e-27 # mass of a proton
correcFactor = protMass / (protMass + eMass) #The correction factor 
rydberg = rydberg * correcFactor # multiply the rydberg constant by the correction

#print the new value for R
print("Rydberg constant:",int(round(rydberg)), "m"+chr(8315)+chr(185))
...
# Compute the subtraction of the experimental data from the simulation
diff = np.subtract(nist, bohrWave* 1e9)

# Create a bar plot that depcits the difference between experimental and simulation
color = '#bf00bf' #hexcode to color match
plt.bar(ni, diff, color=color, label = 'NIST-Bohr')
...
worstCaseError = np.max(np.abs(diff))
worstCaseError = "{:.3f}".format(worstCaseError)
print("Worst-case error:", worstCaseError,"nm")